...
ENTRY_RISK_PAUSE_LOG_SEC = max(60, int(os.getenv("ENTRY_RISK_PAUSE_LOG_SEC", "300")))
...
LIVE_RECORD_BLOCKED_SHADOW_SAMPLES = os.getenv("LIVE_RECORD_BLOCKED_SHADOW_SAMPLES", "true").lower() == "true"
LIVE_BLOCKED_SHADOW_MAX_OPEN = max(0, int(os.getenv("LIVE_BLOCKED_SHADOW_MAX_OPEN", "250")))
LIVE_BLOCKED_SHADOW_COOLDOWN_SEC = max(0, int(os.getenv("LIVE_BLOCKED_SHADOW_COOLDOWN_SEC", "3600")))
ENABLE_AUTONOMOUS_EDGE_FILTER_SHADOW = (
    os.getenv("ENABLE_AUTONOMOUS_EDGE_FILTER_SHADOW", "true").lower() == "true"
)
AUTONOMOUS_EDGE_FILTER_EXPERIMENT_KEY = "esports_edge_filter_shadow_v2"
AUTONOMOUS_ESPORTS_EDGE_FILTER_EXPERIMENT_KEY = AUTONOMOUS_EDGE_FILTER_EXPERIMENT_KEY
AUTONOMOUS_SPORTS_EDGE_FILTER_EXPERIMENT_KEY = "sports_edge_filter_shadow_v1"
AUTONOMOUS_RETIRED_EDGE_FILTER_EXPERIMENT_KEYS = ("esports_edge_filter_shadow_v1",)
AUTONOMOUS_ACTIVE_EDGE_FILTER_EXPERIMENT_KEYS = (
    AUTONOMOUS_ESPORTS_EDGE_FILTER_EXPERIMENT_KEY,
    AUTONOMOUS_SPORTS_EDGE_FILTER_EXPERIMENT_KEY,
)
AUTONOMOUS_EDGE_FILTER_EXPERIMENT_KEYS = (
    *AUTONOMOUS_RETIRED_EDGE_FILTER_EXPERIMENT_KEYS,
    *AUTONOMOUS_ACTIVE_EDGE_FILTER_EXPERIMENT_KEYS,
)
AUTONOMOUS_EDGE_FILTER_MIN_PRICE = float(os.getenv("AUTONOMOUS_EDGE_FILTER_MIN_PRICE", "0.28"))
AUTONOMOUS_EDGE_FILTER_MAX_PRICE = float(os.getenv("AUTONOMOUS_EDGE_FILTER_MAX_PRICE", "0.42"))
AUTONOMOUS_EDGE_FILTER_TARGET_PRICE = float(os.getenv("AUTONOMOUS_EDGE_FILTER_TARGET_PRICE", "0.34"))
AUTONOMOUS_EDGE_FILTER_MIN_LIQUIDITY = float(os.getenv("AUTONOMOUS_EDGE_FILTER_MIN_LIQUIDITY", "2000"))
AUTONOMOUS_EDGE_FILTER_MIN_LEAD_SEC = int(os.getenv("AUTONOMOUS_EDGE_FILTER_MIN_LEAD_SEC", "3600"))
AUTONOMOUS_EDGE_FILTER_MAX_LEAD_SEC = int(os.getenv("AUTONOMOUS_EDGE_FILTER_MAX_LEAD_SEC", "129600"))
AUTONOMOUS_EDGE_FILTER_MIN_SCORE = float(os.getenv("AUTONOMOUS_EDGE_FILTER_MIN_SCORE", "74"))
AUTONOMOUS_ESPORTS_EDGE_FILTER_MIN_PRICE = float(
    os.getenv("AUTONOMOUS_ESPORTS_EDGE_FILTER_MIN_PRICE", "0.43")
)
AUTONOMOUS_ESPORTS_EDGE_FILTER_MAX_PRICE = float(
    os.getenv("AUTONOMOUS_ESPORTS_EDGE_FILTER_MAX_PRICE", "0.47")
)
AUTONOMOUS_ESPORTS_EDGE_FILTER_TARGET_PRICE = float(
    os.getenv("AUTONOMOUS_ESPORTS_EDGE_FILTER_TARGET_PRICE", "0.46")
)
AUTONOMOUS_ESPORTS_EDGE_FILTER_MIN_LIQUIDITY = float(
    os.getenv("AUTONOMOUS_ESPORTS_EDGE_FILTER_MIN_LIQUIDITY", "5000")
)
AUTONOMOUS_ESPORTS_EDGE_FILTER_MIN_LEAD_SEC = int(
    os.getenv("AUTONOMOUS_ESPORTS_EDGE_FILTER_MIN_LEAD_SEC", "1800")
)
AUTONOMOUS_ESPORTS_EDGE_FILTER_MAX_LEAD_SEC = int(
    os.getenv("AUTONOMOUS_ESPORTS_EDGE_FILTER_MAX_LEAD_SEC", "43200")
)
AUTONOMOUS_ESPORTS_EDGE_FILTER_MIN_SCORE = float(
    os.getenv("AUTONOMOUS_ESPORTS_EDGE_FILTER_MIN_SCORE", "82")
)
AUTONOMOUS_EDGE_FILTER_MAX_SIGNALS_PER_CYCLE = max(
    0,
    int(os.getenv("AUTONOMOUS_EDGE_FILTER_MAX_SIGNALS_PER_CYCLE", "2")),
)
AUTONOMOUS_EDGE_FILTER_MIN_DECIDED_SAMPLES = max(
    1,
    int(os.getenv("AUTONOMOUS_EDGE_FILTER_MIN_DECIDED_SAMPLES", "50")),
)
AUTONOMOUS_EDGE_FILTER_ROLLBACK_MIN_DECIDED = max(
    1,
    int(os.getenv("AUTONOMOUS_EDGE_FILTER_ROLLBACK_MIN_DECIDED", "30")),
)
AUTONOMOUS_EDGE_FILTER_ROLLBACK_MAX_WIN_RATE = float(
    os.getenv("AUTONOMOUS_EDGE_FILTER_ROLLBACK_MAX_WIN_RATE", "0.45")
)
ENABLE_COPY_ARCHIVE_SHADOW = os.getenv("ENABLE_COPY_ARCHIVE_SHADOW", "true").lower() == "true"
COPY_ARCHIVE_SHADOW_EXPERIMENT_KEY = "sports_copy_archive_shadow_v1"
COPY_ARCHIVE_SHADOW_SCOPE = os.getenv("COPY_ARCHIVE_SHADOW_SCOPE", "sports").strip().lower() or "sports"
COPY_ARCHIVE_SHADOW_TRADERS = tuple(
    item.strip()
    for item in os.getenv(
        "COPY_ARCHIVE_SHADOW_TRADERS",
        "0xd106952ebf30a3125affd8a23b6c1f30c35fc79c|Herdonia|85",
    ).split(",")
    if item.strip()
)
COPY_ARCHIVE_SHADOW_MAX_SIGNALS_PER_CYCLE = max(
    0,
    int(os.getenv("COPY_ARCHIVE_SHADOW_MAX_SIGNALS_PER_CYCLE", "2")),
)
COPY_ARCHIVE_SHADOW_MAX_SIGNAL_AGE_SEC = max(
    60,
    int(os.getenv("COPY_ARCHIVE_SHADOW_MAX_SIGNAL_AGE_SEC", "900")),
)
COPY_ARCHIVE_SHADOW_SIMULATED_MAX_TRADE_VALUE_USDC = max(
    0.0,
    float(os.getenv("COPY_ARCHIVE_SHADOW_SIMULATED_MAX_TRADE_VALUE_USDC", "3.00")),
)
COPY_ARCHIVE_SHADOW_MIN_DECIDED_SAMPLES = max(
    1,
    int(os.getenv("COPY_ARCHIVE_SHADOW_MIN_DECIDED_SAMPLES", "50")),
)
COPY_ARCHIVE_SHADOW_ROLLBACK_MIN_DECIDED = max(
    1,
    int(os.getenv("COPY_ARCHIVE_SHADOW_ROLLBACK_MIN_DECIDED", "30")),
)
COPY_ARCHIVE_SHADOW_ROLLBACK_MAX_WIN_RATE = float(
...
SHADOW_RECOVERY_ACTIVE_EXPERIMENT_KEYS = (
    *AUTONOMOUS_ACTIVE_EDGE_FILTER_EXPERIMENT_KEYS,
    COPY_ARCHIVE_SHADOW_EXPERIMENT_KEY,
)
SHADOW_RECOVERY_EXPERIMENT_KEYS = (
    *SHADOW_RECOVERY_RETIRED_EXPERIMENT_KEYS,
    *SHADOW_RECOVERY_ACTIVE_EXPERIMENT_KEYS,
)
...
ENABLE_AUTONOMOUS_LOSS_PROBATION = os.getenv("ENABLE_AUTONOMOUS_LOSS_PROBATION", "true").lower() == "true"
AUTONOMOUS_LOSS_PROBATION_LOOKBACK_DAYS = max(
    1,
    int(os.getenv("AUTONOMOUS_LOSS_PROBATION_LOOKBACK_DAYS", "3")),
)
AUTONOMOUS_LOSS_PROBATION_MIN_DECISIONS = max(
    1,
    int(os.getenv("AUTONOMOUS_LOSS_PROBATION_MIN_DECISIONS", "8")),
)
AUTONOMOUS_LOSS_PROBATION_MAX_WIN_RATE = float(os.getenv("AUTONOMOUS_LOSS_PROBATION_MAX_WIN_RATE", "0.20"))
AUTONOMOUS_LOSS_PROBATION_MAX_OPEN_POSITIONS = max(
    0,
    int(os.getenv("AUTONOMOUS_LOSS_PROBATION_MAX_OPEN_POSITIONS", "1")),
)
ENABLE_AUTONOMOUS_LOSS_QUARANTINE = os.getenv("ENABLE_AUTONOMOUS_LOSS_QUARANTINE", "true").lower() == "true"
AUTONOMOUS_LOSS_QUARANTINE_MIN_DECISIONS = max(
    1,
    int(os.getenv("AUTONOMOUS_LOSS_QUARANTINE_MIN_DECISIONS", "8")),
)
AUTONOMOUS_LOSS_QUARANTINE_MAX_WIN_RATE = float(os.getenv("AUTONOMOUS_LOSS_QUARANTINE_MAX_WIN_RATE", "0.12"))
AUTONOMOUS_LOSS_QUARANTINE_MIN_REALIZED_LOSS_USDC = max(
    0.0,
    float(os.getenv("AUTONOMOUS_LOSS_QUARANTINE_MIN_REALIZED_LOSS_USDC", "1.00")),
)
