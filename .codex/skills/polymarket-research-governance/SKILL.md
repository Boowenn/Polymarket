...
14. On tiny live bankrolls, do not force every autonomous entry to ride all the way to settlement.
   ...
   Current recovery experiment: `esports_edge_filter_shadow_v1` replaces the failed price-only esports market-first selector with a stricter price/liquidity/time/score filter. It is no-money only, writes `sample_type='shadow'` with `experiment_key='esports_edge_filter_shadow_v1'`, respects `LIVE_BLOCKED_SHADOW_MAX_OPEN` and `LIVE_BLOCKED_SHADOW_COOLDOWN_SEC`, has `$0` real-money exposure, requires at least `50` decided samples before review, and rolls back if after `30` decided samples win rate is at or below `45%` or PnL is negative.
...
- Never let the `esports_edge_filter_shadow_v1` quarantine experiment place orders or consume live budget; it must stay no-money shadow until executed evidence later justifies a separately reviewed live canary.
