...
14. On tiny live bankrolls, do not force every autonomous entry to ride all the way to settlement.
   ...
   Current active recovery experiment: `sports_copy_archive_shadow_v1` is no-money only, writes `sample_type='shadow'`, respects `LIVE_BLOCKED_SHADOW_MAX_OPEN` and `LIVE_BLOCKED_SHADOW_COOLDOWN_SEC`, has `$0` real-money exposure, requires at least `50` decided samples before review, and rolls back if after `30` decided samples win rate is at or below `45%` or PnL is negative. `esports_edge_filter_shadow_v1`, `esports_edge_filter_shadow_v2`, and `sports_edge_filter_shadow_v1` remain in reports as retired comparison evidence after poor or failed no-money reads; no active edge-filter shadow should write new rows until a new plan includes a clearly different edge source. `sports_copy_archive_shadow_v1` continues the archived sports copy-trading recovery hypothesis from the Tier-A archive seed only; it excludes esports by default, may simulate a minimum executable paper size within `COPY_ARCHIVE_SHADOW_SIMULATED_MAX_TRADE_VALUE_USDC`, and cannot be promoted directly from archived or shadow PnL. `sports_copy_archive_live_canary_v1` has disabled-by-default wiring and dashboard/report separation, but it must stay off unless both `ENABLE_COPY_ARCHIVE_LIVE_CANARY=true` and `COPY_ARCHIVE_LIVE_CANARY_OPERATOR_APPROVED=true` are deliberately set in local `.env` after the documented pre-enable checks.
...
- Never let the active `sports_copy_archive_shadow_v1` quarantine experiment place orders or consume live budget; it must stay no-money shadow until executed evidence later justifies a separately reviewed live canary. Keep retired `esports_edge_filter_shadow_v1`, `esports_edge_filter_shadow_v2`, and `sports_edge_filter_shadow_v1` visible only as old shadow evidence, not as active recovery rules.
