...
- while loss quarantine is active, collect no-money `shadow` only edge-filter samples: `esports_edge_filter_shadow_v1` for esports moneylines and `sports_edge_filter_shadow_v1` for sports moneylines. Both use the stricter price/liquidity/time/score window, write no orders, respect `LIVE_BLOCKED_SHADOW_MAX_OPEN` and `LIVE_BLOCKED_SHADOW_COOLDOWN_SEC`, and require at least `AUTONOMOUS_EDGE_FILTER_MIN_DECIDED_SAMPLES` before any live recovery review
...
- current recovery experiments: `esports_edge_filter_shadow_v1` tracks esports moneylines and `sports_edge_filter_shadow_v1` tracks sports moneylines under the same no-money edge filter. Both record `sample_type='shadow'`, keep separate `experiment_key` values, have maximum real-money exposure `$0`, wait for at least `50` decided samples before review, and roll back if after `30` decided samples win rate is at or below `45%` or PnL is negative
