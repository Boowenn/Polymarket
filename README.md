...
- while loss quarantine is active, collect no-money `shadow` only edge-filter samples: retired esports v1 and v2 stay visible for review, and active sports writes `sports_edge_filter_shadow_v1`. They write no orders, respect `LIVE_BLOCKED_SHADOW_MAX_OPEN` and `LIVE_BLOCKED_SHADOW_COOLDOWN_SEC`, and require at least `AUTONOMOUS_EDGE_FILTER_MIN_DECIDED_SAMPLES` before any live recovery review
...
- current recovery experiments: `sports_edge_filter_shadow_v1` keeps tracking traditional sports moneylines under the original edge filter, and `sports_copy_archive_shadow_v1` continues the archived sports copy-trading recovery. `esports_edge_filter_shadow_v1` and `esports_edge_filter_shadow_v2` are both retained only as retired comparison rows after poor no-money reads; no active esports edge-filter shadow is writing new rows. Active recovery rows record `sample_type='shadow'`, keep separate `experiment_key` values, have maximum real-money exposure `$0`, wait for at least `50` decided samples before review, and roll back if after `30` decided samples win rate is at or below `45%` or PnL is negative
- current copy recovery experiment: `sports_copy_archive_shadow_v1` continues the archived sports copy-trading strategy as no-money shadow only. It is deliberately separate from the autonomous edge-filter tracks, excludes esports by default, and starts from the archived Tier-A seed `Herdonia`. Do not promote it directly from archived or shadow PnL; a later real-money copy canary needs its own reviewed plan and executed evidence
...
- `AUTONOMOUS_EDGE_FILTER_MIN_PRICE` / `AUTONOMOUS_EDGE_FILTER_MAX_PRICE`, `AUTONOMOUS_EDGE_FILTER_MIN_LIQUIDITY`, `AUTONOMOUS_EDGE_FILTER_MIN_SCORE`, and lead-time bounds to define the traditional sports shadow rule; `AUTONOMOUS_ESPORTS_EDGE_FILTER_*` defines the separate esports v2 no-money rule.
