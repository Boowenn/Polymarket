...
- while loss quarantine is active, do not write new edge-filter shadow samples. Retired esports v1/v2 and sports v1 stay visible only as historical comparison rows after poor or failed no-money reads
- while loss quarantine is active, continue the archived sports copy-trading recovery as no-money shadow only with `sports_copy_archive_shadow_v1`. It follows only the archived Tier-A sports seed by default, writes `sample_type='shadow'`, places no orders, has maximum real-money exposure `$0`, and can simulate a minimum executable sports size under `COPY_ARCHIVE_SHADOW_SIMULATED_MAX_TRADE_VALUE_USDC` so tiny live caps do not prevent paper-only evidence collection. It still needs at least `COPY_ARCHIVE_SHADOW_MIN_DECIDED_SAMPLES` decided samples before any live canary review
...
- current recovery experiment: `sports_copy_archive_shadow_v1` continues the archived sports copy-trading recovery. `esports_edge_filter_shadow_v1`, `esports_edge_filter_shadow_v2`, and `sports_edge_filter_shadow_v1` are retained only as retired comparison rows after poor or failed no-money reads; no active edge-filter shadow is writing new rows. Active recovery rows record `sample_type='shadow'`, have maximum real-money exposure `$0`, wait for at least `50` decided samples before review, and roll back if after `30` decided samples win rate is at or below `45%` or PnL is negative
- current copy recovery experiment: `sports_copy_archive_shadow_v1` continues the archived sports copy-trading strategy as no-money shadow only. It is deliberately separate from the autonomous edge-filter tracks, excludes esports by default, and starts from the archived Tier-A seed `Herdonia`. Do not promote it directly from archived or shadow PnL; the reviewed real-money copy canary plan is kept in `docs/SPORTS_COPY_ARCHIVE_LIVE_CANARY_PLAN.md`. The disabled-by-default implementation writes executed rows under `sports_copy_archive_live_canary_v1` only if both `ENABLE_COPY_ARCHIVE_LIVE_CANARY=true` and `COPY_ARCHIVE_LIVE_CANARY_OPERATOR_APPROVED=true` are set in local `.env`, and it keeps `signal_source='copy_archive_canary'` separate from default autonomous entries
...
- `ENABLE_AUTONOMOUS_EDGE_FILTER_SHADOW=false` because the prior esports and sports edge-filter shadows are retired; only enable it again with a new no-money plan and a new experiment key.
- `AUTONOMOUS_EDGE_FILTER_MIN_PRICE` / `AUTONOMOUS_EDGE_FILTER_MAX_PRICE`, `AUTONOMOUS_EDGE_FILTER_MIN_LIQUIDITY`, `AUTONOMOUS_EDGE_FILTER_MIN_SCORE`, and lead-time bounds remain available for a future edge-filter plan, but no current active edge-filter key writes new rows.
- `ENABLE_COPY_ARCHIVE_SHADOW=true` to continue archived sports copy-trading as no-money shadow only in live quarantine, using `COPY_ARCHIVE_SHADOW_TRADERS`, `COPY_ARCHIVE_SHADOW_SCOPE`, `COPY_ARCHIVE_SHADOW_MAX_SIGNAL_AGE_SEC`, `COPY_ARCHIVE_SHADOW_SIMULATED_MAX_TRADE_VALUE_USDC`, and `COPY_ARCHIVE_SHADOW_MAX_SIGNALS_PER_CYCLE`.
- `ENABLE_COPY_ARCHIVE_LIVE_CANARY=false` and `COPY_ARCHIVE_LIVE_CANARY_OPERATOR_APPROVED=false` keep the archived-copy real-money canary off until an operator deliberately enables both switches. When enabled, `COPY_ARCHIVE_LIVE_CANARY_MAX_GROSS_USDC=4.50`, `COPY_ARCHIVE_LIVE_CANARY_MAX_TRADE_VALUE_USDC=1.50`, `COPY_ARCHIVE_LIVE_CANARY_MAX_OPEN_POSITIONS=1`, `COPY_ARCHIVE_LIVE_CANARY_MAX_ENTRIES=5`, and `COPY_ARCHIVE_LIVE_CANARY_COOLDOWN_SEC=21600` are the first-pass caps from the canary plan.
