...
- while loss quarantine is active, collect no-money `shadow` only edge-filter samples: active esports writes `esports_edge_filter_shadow_v2`, retired esports v1 stays visible for review, and sports writes `sports_edge_filter_shadow_v1`. They write no orders, respect `LIVE_BLOCKED_SHADOW_MAX_OPEN` and `LIVE_BLOCKED_SHADOW_COOLDOWN_SEC`, and require at least `AUTONOMOUS_EDGE_FILTER_MIN_DECIDED_SAMPLES` before any live recovery review
...
- current recovery experiments: `esports_edge_filter_shadow_v2` tracks esports moneylines with a narrower no-money near-consensus window, while `sports_edge_filter_shadow_v1` keeps tracking traditional sports moneylines under the original edge filter. `esports_edge_filter_shadow_v1` is retained as a retired comparison row after the poor early read. All record `sample_type='shadow'`, keep separate `experiment_key` values, have maximum real-money exposure `$0`, wait for at least `50` decided samples before review, and roll back if after `30` decided samples win rate is at or below `45%` or PnL is negative
