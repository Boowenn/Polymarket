...
- current copy recovery experiment: `sports_copy_archive_shadow_v1` continues the archived sports copy-trading strategy as no-money shadow only. It is deliberately separate from the autonomous edge-filter tracks, excludes esports by default, and starts from the archived Tier-A seed `Herdonia`. Do not promote it directly from archived or shadow PnL; the reviewed draft for a later real-money copy canary is kept in `docs/SPORTS_COPY_ARCHIVE_LIVE_CANARY_PLAN.md`, and it is not enabled until separately implemented and explicitly approved
