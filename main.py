...
def first_run_setup():
    ...
    print("    [1] Watch Mode  - just show top trader signals (no money needed)")
    print("    [2] Live Mode   - auto-mirror trades to your wallet")
    print()
    ...
    if choice == "2":
        ...
        stake = input("  Stake % of whale trade [1]: ").strip() or "1"
        dry_run = "false"
    else:
        pk = ""
        funder = ""
    ...
    with open(env_path, "w") as f:
        ...
        f.write(f"ENABLE_CONSENSUS_STRATEGY=true\n")
...
def run_cycle(cycle_count):
    ...
    # 3. If copy flow is quiet, try the conservative consensus strategy
    strategy_signals = []
    if not new_signals:
        try:
    ...
    # 4. Execute each signal
    for signal in new_signals + strategy_signals:
        try:
    ...
    # 5. Reconcile natural market settlements for still-open journal entries
    try:
